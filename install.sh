...
for arg in "$@"; do
    case $arg in
        ...
        --help)
            echo "사용법: $0 [옵션]"
            echo "  --torch-cuda=VER   pip wheel CUDA 버전 (기본: cu128)"
...
if ! grep -qi "ubuntu" /etc/os-release 2>/dev/null; then
    warn "Ubuntu 22.04 기준 스크립트입니다. 다른 OS에서는 패키지명이 다를 수 있습니다."
fi
...
# =============================================================================
# 3. 시스템 CUDA Toolkit (apt) — ffmpeg NVENC/NVDEC용
# =============================================================================
...
if dpkg -l 2>/dev/null | grep -q "cuda-toolkit-[0-9]"; then
    ...
    warn "    드라이버 520.x → CUDA 12.0"
    warn "    드라이버 525.x → CUDA 12.0"
...
# =============================================================================
# 6. PyTorch + CUDA (pip) — Python 패키지용
# =============================================================================
...
if [ "$SKIP_TORCH" = true ]; then
    ...
    info "PyTorch 이미 설치됨: $TORCH_VER (건너뜀. 재설치: --skip-torch 없이 실행)"
else
    ...
    echo "  설치 인덱스: $TORCH_INDEX"
    "$VENV_PIP" install torch torchvision torchaudio --index-url "$TORCH_INDEX"
    info "PyTorch 설치 완료"
fi
